...
def create_catch(catch: CatchCreate, db: Session = next(get_db())):
...
def get_all_catches(db: Session = next(get_db()), catch_type: str = None):
...
def get_catch(catch_id: int, db: Session = next(get_db())):
...
def update_catch(catch_id: int, catch_update: CatchUpdate, db: Session = next(get_db())):
    catch = db.query(Catch).filter(Catch.id == catch_id).first()
    if not catch:
        raise HTTPException(status_code=404, detail="Catch not found")
...
def delete_catch(catch_id: int, db: Session = next(get_db())):
    catch = db.query(Catch).filter(Catch.id == catch_id).first()
    if not catch:
        raise HTTPException(status_code=404, detail="Catch not found")
